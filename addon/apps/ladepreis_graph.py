...
class LadepreisGraph(hass.Hass):
    ...
    def _render_preview(self, ladepreis, smiley_img, surplus_m, surplus_u, surplus_3):
        ...
        ct(240, 30,  f"{ladepreis:.1f}" if ladepreis is not None else "—",
                                                                       True,  180)
    ...
    def render_combined(self, *args, **kwargs):
        ...
        try:
            ...
            self.call_service("input_number/set_value",
                              entity_id="input_number.nl_surplus_morgen",
                              value=round(max(0.0, surplus_m), 1))
            self.call_service("input_number/set_value",
                              entity_id="input_number.nl_surplus_uebermorgen",
                              value=round(max(0.0, surplus_u), 1))
            self.call_service("input_number/set_value",

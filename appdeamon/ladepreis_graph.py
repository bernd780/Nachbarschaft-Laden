...
try:
    ...
    _TZ = timezone(timedelta(hours=2))  # CEST-Fallback
...
class LadepreisGraph(hass.Hass):
    ...
    W       = 460   # Breite des Smileys-Bilds
    H_TOTAL = 160   # Hoehe des Smileys-Bilds (eng, kein Leerraum)
    ...
    DISPLAY_W    = 480
    DISPLAY_H    = 800
    QR_URL       = "https://nachbarschaft-laden.de/local/nachbarschaft-laden/index.html"
    HISTORY_FILE = "/homeassistant/www/nachbarschaft-laden/price_history.json"
    HISTORY_HOURS = 72

    def initialize(self):
        self.log("LadepreisGraph gestartet")
    ...
    def _render_preview(self, ladepreis, smiley_img, surplus_m, surplus_u, surplus_3):
        ...
        # ── Ladepreis (y=0–296) – gleiche visuelle Abstände à 29px ──
        ...
        ct(240, 304, "PV-Überschuss Prognose", True, 22)
        ct(cx0, 334, "Morgen",                 True, 22)
        ct(cx1, 334, "Übermorgen",             True, 22)
        ct(cx2, 334, "In 3 Tagen",             True, 22)
        ...
        # ── Logo (460x145, zentriert, y=655–800) – zuerst zeichnen, damit QR darüber liegt ──
        LOGO_W, LOGO_H = 460, 145
        logo_drawn = False
        try:
            from PIL import ImageOps
            logo_img = Image.open("/homeassistant/www/nachbarschaft-laden/logo_display.png").convert("RGB")
        ...
        if not logo_drawn:
            # Fallback: PIL-gezeichnetes Logo
            lx, ly = 20, 665
            ...
            ct(230, 670, "Nachbarschaft-", True, 26)
            ct(230, 708, "Laden",          True, 34)

        # ── Uhrzeit + QR (y=570–655) – nach Logo zeichnen, damit QR im Vordergrund liegt ──
        now = datetime.now()
        ct(130, 588, now.strftime("%H:%M"),    True,  28)
        ct(130, 622, now.strftime("%d.%m.%Y"), False, 20)
        ...
        out = "/homeassistant/www/nachbarschaft-laden/display_preview.png"
        tmp = out + ".tmp"
        img.save(tmp, "PNG")
        os.replace(tmp, out)
    ...
    def render_combined(self, kwargs):
        ...
        # Ladepreis (wird auf dem Display angezeigt)
        ladepreis = None
        try:
            v = self.get_state("sensor.expgldurchschnitt_ema_asymalpha")
            if v not in (None, "unavailable", "unknown"):
                ladepreis = float(v)
        except Exception:
            pass

        # PV-Sensoren laden (jeder Sensor einzeln, damit ein unavailable-Wert
        # nicht alle anderen Werte auf 0 setzt)
        pv_m = self._float_safe("sensor.morgenpv")
        pv_u = self._float_safe("sensor.uebermorgenpv")
        pv_3 = self._float_safe("sensor.pvin3tagen")
        soc  = self._float_safe("sensor.mein_fahrzeug_battery")
        haus = self._float_safe("input_number.hausverbrauchtgl")
        ziel = self._float_safe("input_number.autoakkuziel", default=80.0)
        ...
        # Bild: nur 3 Smileys nebeneinander, kein Text
        ...
        # Display-Preview (vollständiges Layout, nicht invertiert)
        try:
        ...
        try:
            ...
            # Eigene History laden, aktuellen Wert anhängen, auf 72h trimmen
            ...
            # Sparkline: letzte 3h, 15min-Buckets
            ...
            # Vortag-Durchschnitt: direkt aus price_history.json (Lokalzeit)
            ...
            # Günstigste Stunde heute (13–19 Uhr): gleitendes 60-Minuten-Fenster
            ...
            # Heutige PV-Erzeugung vs. Ertragsprognose
            pv_erzeugung_prozent = None
            try:
                v = self.get_state("sensor.prozentpverzeugungheute")
                if v not in (None, "unavailable", "unknown"):
                    pv_erzeugung_prozent = round(float(v), 1)
            except Exception:
                pass
            ...
            ladevorgang = {"aktiv": False, "verbunden": False,
                           ...
                           "kosten_session": None}
            try:
                car = self.get_state("sensor.go_echarger_XXXXXX_car")
                ladevorgang["aktiv"]     = (car == "Charging")
                ladevorgang["verbunden"] = car in ("Charging", "Complete", "Wait for car")

                # Energie aus ETO-Differenz (genauer als evcc-Session-Sensor)
                try:
                    eto       = float(self.get_state("sensor.go_echarger_XXXXXX_eto") or 0)
                    eto_start = float(self.get_state("input_number.go_echarger_session_eto_start") or 0)
                    energie_kwh = eto - eto_start
                    if energie_kwh > 0:
                        ladevorgang["energie_kwh"] = round(energie_kwh, 2)
                except Exception:
                    # Fallback: evcc-Sensor
                    v = self.get_state("sensor.evcc_go_echarger_ocpp_session_energy")
                    if v not in (None, "unavailable", "unknown"):
                        ladevorgang["energie_kwh"] = round(float(v), 2)

                # Live-Kosten: Integral-Differenz seit Session-Start
                try:
                    integral_now   = float(self.get_state("sensor.go_echarger_kosten_integral_2") or 0)
                    integral_start = float(self.get_state("input_number.go_echarger_kosten_integral_start") or 0)
                    if integral_now > integral_start:
                        ladevorgang["kosten_session"] = round(integral_now - integral_start, 4)
                except Exception:
                    pass

                for key, sensor, digits in [
                    ("dauer_s", "sensor.evcc_go_echarger_ocpp_charge_duration", 0),
                    ("soc",     "sensor.evcc_go_echarger_ocpp_vehicle_soc",     0),
                ]:
            ...
            json_path = "/homeassistant/www/nachbarschaft-laden/data.json"
